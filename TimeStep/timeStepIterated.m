%%%% This notebook contains the code producing the bifurcation diagram of
%%%% the model.
...
nx = 2^11; % We assume equal number of neurons for both populations and use the same grid for both connectivity kernels.  
Lx = 200; % 200 mm!
...
bifVarMax = [];  % initialize the arrays to store bifurcation variable values
bifVarMin = [];
index = 1535;

bifIn = 0; bifFin = 2.0; 
bifNum = 41; % determines the resulution of the bifurcation diagram
bifParList = linspace(bifIn, bifFin, bifNum);

%% Iterate for the bifurcation diagram
for bInd =1:length(bifParList)
    
    close all;
    bifPar = bifParList(bInd);
    fprintf('Bifurcation parameter: %.2f', bifPar)    
    ...
    p(18) = 1.0;     % c1 
    p(19) = 1.0;     % c2
    p(20) = -0.25;   % cross connection weight from inh. to exc. population (cie)
    p(21) = 0.25;    % cross connection weight from exc. to inh. population (cei)
    ...
    timeOutE = @(t,z,flag) TimeOutputEforPropagationSpeed(t,z,flag,true,x,p,parentE,idx);
    timeOutI = @(t,z,flag) TimeOutputIforPropagationSpeed(t,z,flag,true,x,p,parentI,idx);
    ...
    id = 1024; % index of the neuron whose plots are shown
    ...
    PlotSolutionFinE(x,Z(end,:),p,[],idx(:, 1),false); % Plot the solution for the exc. population
    % findpeaks(Z(end,idx(:, 1)),idx(:, 1)) % Find the peak points

    PlotSolutionFinI(x,Z(end,:),p,[],idx(:, 2),false); % Plot the solution for the inh. population
    ...
    errorVal = 0.075;
    if minDist>errorVal
        disp('No propagating excitatory wave! Increase the error value.')
    else    
...
%     waveFrontThreshold2 = p(25);
%     dist2    = abs(Z(end,idx(:, 2)) - waveFrontThreshold2);
%     minDist2 = min(dist2);    
%     if minDist2>errorVal
%         disp('No propagating inhibitory wave! Increase the error value.')
%     else    
%         indFind2     = find(dist2 == minDist2);
%         waveFrontIndexI = max(indFind2);
% 
%         % % Choose the global peak 
%         % if length(peakIndex2)>1 
%         % % peakIndex2 = peakIndex2(max(find(peakVal2==max(peakVal2)))); % Choose the right-hand side peak
%         % end    
%         waveFrontIndexI = abs(-LxPhysical + (waveFrontIndexI-1)*hx);
%         waveFrontIndexIReel = waveFrontIndexI/spatialUnitConstant
%         indFind2
%         propagationSpeedInh = waveFrontIndexI/tF % Compute the physical propagation speed and print it on the command window
%     end
    
    bifVarMax = [bifVarMax, max(Z(10:end, index))];
    bifVarMin = [bifVarMin, min(Z(10:end, index))];    
    
end

%% Plot the  bifurcation diagram
figure;
plot(bifParList, bifVarMax,'Linewidth', 2); 
ylabel('ve'); xlabel('kVStar'); title('ve vs c');
hold on
plot(bifParList, bifVarMin,'Linewidth', 2); 